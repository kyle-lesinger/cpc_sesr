...
def return_state_name(Y_degree,X_degree):
    geocoder_key = 'ef485c3651a04d4484b40fac2107e97e' #My personal key with account
    geocoder = OpenCageGeocode(geocoder_key)
    all_info = geocoder.reverse_geocode(Y_degree, X_degree-360)[0]
    keys1 = list(all_info)
    return(all_info['components']['state'])



def return_date_as_text(numpy_datetime_obj):
    a=pd.to_datetime(numpy_datetime_obj)
    a = f'{a.year}-{a.month:02}-{a.day:02}'
    return(a)




def save_with_RZSM(pet_or_refet, all_dates_or_doy, return_date_as_text, fd, num_dates_b4_and_after,idx, vals,state_name, Y, X, year_ranges_tuple,rzsm):

    save_dir = f'{call.fig_dir}/plot_individual_grid_cell_FD_events_with_RZSM/{state_name}_Y_{Y}_X_{X}'
    os.makedirs(save_dir, exist_ok=True)
    
    save_name= f'{save_dir}/fd_with_RZSM_{pet_or_refet}_{all_dates_or_doy}_{return_date_as_text(fd.time.values[vals[0]])}_{return_date_as_text(fd.time.values[vals[-1]])}_for_climatology_{year_ranges_tuple[0]}-{year_ranges_tuple[-1]}.png'
    ...
    delta_sesr_percentile = fd[f'dzSESR_pct_{pet_or_refet}'][dates_before:dates_after,Y,X].values
    sesr_values = fd[f'SESR_{pet_or_refet}'][dates_before:dates_after,Y,X].values
    sesr_percentile = fd[f'SESR_pct_{pet_or_refet}'][dates_before:dates_after,Y,X].values
    rzsm_percentile_values = rzsm['rzsm_dtrnd_pct'][dates_before:dates_after,Y,X].values
    ...
    rapid_intensification_end_value = fd[f'SESR_pct_{pet_or_refet}'][vals[1],Y,X].values
    ...
    mean_delta_sesr = fd[f'dzSESR_pct_{pet_or_refet}'][vals[0]:vals[-1]+1,Y,X].values
    ...
    dt_txt = [pd.to_datetime(return_date_as_text(i)) for i in time_points]
...
def save_without_RZSM(pet_or_refet, all_dates_or_doy, return_date_as_text, fd, num_dates_b4_and_after,idx, vals,state_name, Y, X, year_ranges_tuple):

    save_dir = f'{call.fig_dir}/plot_individual_grid_cell_FD_events_no_RZSM/{state_name}_Y_{Y}_X_{X}'
    os.makedirs(save_dir, exist_ok=True)
    
    save_name= f'{save_dir}/fd_{pet_or_refet}_{all_dates_or_doy}_{return_date_as_text(fd.time.values[vals[0]])}_{return_date_as_text(fd.time.values[vals[-1]])}_for_climatology_{year_ranges_tuple[0]}-{year_ranges_tuple[-1]}.png'
...
def save_with_RZSM_and_USDM(pet_or_refet, all_dates_or_doy, return_date_as_text, fd, num_dates_b4_and_after,idx, vals,state_name, Y, X, year_ranges_tuple,rzsm,usdm):


    save_dir = f'{call.fig_dir}/plot_individual_grid_cell_FD_events_RZSM_USDM/{state_name}_Y_{Y}_X_{X}'
    os.makedirs(save_dir, exist_ok=True)
    
    save_name= f'{save_dir}/fd_USDM_RZSM_{pet_or_refet}_{all_dates_or_doy}_{return_date_as_text(fd.time.values[vals[0]])}_{return_date_as_text(fd.time.values[vals[-1]])}_for_climatology_{year_ranges_tuple[0]}-{year_ranges_tuple[-1]}.png'
    ...
    if pd.to_datetime(fd.time.values[dates_before]).year < 2000:
        ...
        usdm_subset = usdm['fd_2'].sel(time=slice(time_points[0]-np.timedelta64('7','D'),time_points[-1]))
        ...
        if len(indi) == 0:
            ...
            start_USDM = return_date_as_text(usdm_subset.time.values[indi[0]])
            end_USDM = return_date_as_text(usdm_subset.time.values[indi[-1]])
...
def plot_all_FD_events_not_in_winter_no_RZSM(fd,pet_or_refet,Y,X,ones,state_name,year_ranges_tuple,all_dates_or_doy,rzsm,usdm,num_dates_b4_and_after):
    ...
    for idx,vals in enumerate(ones):
        ...
        if (pd.to_datetime(fd.time.values[vals[0]]).month in [12,1,2]) or (pd.to_datetime(fd.time.values[vals[-1]]).month in [12,1,2]):
            ...
            save_with_RZSM(pet_or_refet, all_dates_or_doy, return_date_as_text, fd, num_dates_b4_and_after, idx, vals,state_name, Y, X, year_ranges_tuple,rzsm)
            save_without_RZSM(pet_or_refet, all_dates_or_doy, return_date_as_text, fd, num_dates_b4_and_after,idx, vals,state_name, Y, X, year_ranges_tuple)
            save_with_RZSM_and_USDM(pet_or_refet, all_dates_or_doy, return_date_as_text, fd, num_dates_b4_and_after,idx, vals,state_name, Y, X, year_ranges_tuple,rzsm,usdm)
    return (print('Completed all of the FD events for a particular grid cell'))

    

def run_FD_time_series_plots(all_dates_or_doy, pet_or_refet, year_ranges_tuple, window):
    
    fd= xr.open_dataset(f'Data/Noah/dzSESR_FD_step_4_from_{all_dates_or_doy}_percentile/dzSESR_fd_step_4_from_{all_dates_or_doy}_percentile_window_size_{window}_years_{year_ranges_tuple[0]}-{year_ranges_tuple[-1]}.nc')
    rzsm = xr.open_dataset(f'Data/Noah/climatology_RZSM_percentile/RZSM_percentile_{all_dates_or_doy}_window_size_{window}_years_{year_ranges_tuple[0]}-{year_ranges_tuple[-1]}.nc')
    rzsm = rzsm.sel(time=fd.time.values)
    usdm = xr.open_dataset('/glade/work/klesinger/sesr/Data/USDM/fd_USDM_classification_0.50_degrees.nc')
    # for k,v in enumerate(list(fd.keys())):
    #     print(v)
    select_lat_lon = [(31,76),(40,30), (35,60),(32,70), (20,20), (24,31), (16,90), (16,80),(31,80),]
    
    # select_lat_lon = [(31,76)]

    for lat_lon in select_lat_lon:
        ...
        state_name = return_state_name(Y_degree,X_degree)
        print(f'Working on index lat Y {Y} and X {X} for state/region {state_name}.')
        varname=f'fd_{pet_or_refet}_s5' #set as the last step of FD that has been run currently.
        ...
        plot_all_FD_events_not_in_winter_no_RZSM(fd,pet_or_refet,Y,X,ones,state_name,year_ranges_tuple,all_dates_or_doy,rzsm,usdm,num_dates_b4_and_after)
    return 0
    ...
    for i, (date, usdm_value) in enumerate(zip(time_points, usdm_classification)):
        ...
        if (usdm_value == 1):
            if num_stars ==0:
                ...
                # ax[0].text(date, sesr_values[i] - 0.5, return_date_as_text(date), ha='center', va='top', fontsize=8, color='gold')
                num_stars+=1
