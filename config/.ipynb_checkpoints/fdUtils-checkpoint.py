...
def FD_step_1(window, year_ranges_tuple_1, year_ranges_tuple_2):

    minimum_length_in_weeks = 4 #Event must last at least 4 weeks in length to be considered a FD
    save_dir = f'Data/dzSESR_FD_step_1'
    ...
    for year_ranges_tuple in [year_ranges_tuple_1, year_ranges_tuple_2]:
        ...
        save_sesr_percentile = f'{save_dir}/SESR_fd_step_1_window_size_{window}_years_{year_ranges_tuple[0]}-{year_ranges_tuple[1]}.nc'
        if os.path.exists(save_sesr_percentile):
            print(f'Completed SESR_fd_step_1_window_size_{window}_years_{year_ranges_tuple[0]}-{year_ranges_tuple[1]}.nc')
            pass
        else:
            #Load data
            delta_sesr = xr.open_dataset(f'Data/SESR_percentile/SESR_percentile_window_size_{window}_years_{year_ranges_tuple[0]}-{year_ranges_tuple[1]}.nc').load()
            delta_sesr['fd_sesr'] = delta_sesr['ESR'].copy(deep=True)
            delta_sesr['fd_sesr'][:,:,:] = 0
            
            wednesdays = delta_sesr.sel(time=delta_sesr.time.dt.dayofweek == 2)
            wednesdays['mean_sesr_change'] = wednesdays['ESR'].copy(deep=True)
            wednesdays['mean_sesr_change'][:,:,:] = np.nan
            
            wednesdays['mean_delta_sesr_change'] = wednesdays['mean_sesr_change'].copy(deep=True)
            # wednesdays['delta_sesr_fd_pct'] = wednesdays['mean_sesr_change'].copy(deep=True)
            ...
            add_data_to_fd_index_before = np.array(wednesdays.fd_sesr.values)
            add_data_to_fd_index_after = np.array(wednesdays.fd_sesr.values)
            # print(np.count_nonzero(add_data_to_fd_index_before))
            
            print(f'\nWindow size {window}. Years_{year_ranges_tuple[0]}-{year_ranges_tuple[1]}.\nChecking if length >= {minimum_length_in_weeks} weeks, SESR less than 20th percentile, and delta_SESR less than 40th percentile for years_{year_ranges_tuple[0]}-{year_ranges_tuple[1]}.')
            '''Check if SESR is less than 20th and delta_SESR is less than 40th'''
            
            delta_sesr_percentile_vals,sesr_percentile_vals = wednesdays['delta_SESR_pct'].values,wednesdays['SESR_pct'].values
            ...
            for Y,_ in enumerate(range(wednesdays.lat.shape[0])):
                for X,_ in enumerate(range(wednesdays.lon.shape[0])):
                    if ~np.isnan(land_mask[Y,X]):
                        #This is the first pass at getting all the data into binary format
                        wednesdays = classify_FD_step_1(Y,X,wednesdays,delta_sesr_percentile_vals,sesr_percentile_vals,min_delta_SESR,min_SESR,minimum_length_in_weeks,growing_start,growing_end)
            wednesdays.to_netcdf(save_sesr_percentile)
    return(f'Completed making step 1 FD for window size {window}.')        

def classify_FD_step_1(Y,X,wednesdays,delta_sesr_percentile_vals,sesr_percentile_vals,min_delta_SESR,min_SESR,minimum_length_in_weeks,growing_start,growing_end):
    new_flash = 0
    continuous_flash = 0
    was_flash = 0
    was_interrupted = 0
    ...
    for idx,week_val in enumerate(wednesdays.time.values):
        
        if (idx <=minimum_length_in_weeks) or (idx == len(wednesdays.time.values)-1):
            ...
            if new_flash != 1 and continuous_flash != 1:
                ...
                if delta_sesr_percentile_vals[idx,Y,X] < min_delta_SESR:
                    if flash_interruption == 1:
                        if wednesdays['SESR'][idx + 1,Y,X].values < wednesdays['SESR'][idx - 1,Y,X].values:
                            if length_flash_drought == 0:
            ...
            if was_flash == 1:
                ...
                start_month = pd.to_datetime(wednesdays.time.values[idx]).month
                
                if (current_month >= growing_start and current_month <= growing_end) or (start_month >= growing_start and start_month <= growing_end) :
                    in_grow_seas = 1
                else:
                    in_grow_seas = 0
                ...
                if (np.any(sesr_percentile_vals[fd_n_end- 4:fd_n_end - 1,Y,X] <=20) and (length_flash_drought >= minimum_length_in_weeks) and (in_grow_seas == 1) and (np.nanmean(wednesdays['delta_SESR_pct'][fd_n_begin:fd_n_end - 1,Y,X].values) <= 25)):
                    # mean_sesr_change = np.nanmean(wednesdays['SESR_detrend_pct'][fd_n_begin:fd_n_end - 1,Y,X].values)
                    # mean_delta_sesr_change = np.nanmean(wednesdays['delta_SESR_detrend_pct'][fd_n_begin:fd_n_end - 1,Y,X].values)

                    # wednesdays['mean_sesr_change'][fd_n_begin:fd_n_end - 1,Y,X] = mean_sesr_change
                    # wednesdays['mean_delta_sesr_change'][fd_n_begin:fd_n_end - 1,Y,X] = mean_delta_sesr_change

                    wednesdays['fd_sesr'][fd_n_begin:fd_n_end - 1,Y,X] = 1
                else:
                    wednesdays['fd_sesr'][fd_n_begin:fd_n_end - 1,Y,X] = 0
